...
# set random_control = True for SFig2C
...
if random_control:
    ob_conn = shuffle_glom_kc_w_prob(ob_conn, glom_prob)
...
# set random_control = True for Fig4C
random_control = False
...
if random_control:
    ob_conn = shuffle_glom_kc_iterate(ob_conn, 1)[0]
...
##------------------------------
# Fig5D one instance of local random vs. random bouton
def randomize_neighbours(ana, pick_fun):
    # given a ana instance and a pick_fun
    # e.g. ana = ana_bd
    # pick_fun = pick_random_from_neighbours, with n initialized
    conn_to_pick = ana.conn_data['bouton_claw'].conn['5s']
    geom_to_pick = ana.geo_data['centroids'].conn
    permuted = pick_partner(conn_to_pick, geom_to_pick, pick_fun)
    col_ids = ana.conn_data['bouton_claw'].col_ids
    row_ids = ana.conn_data['bouton_claw'].row_ids

    t_conn = permuted.copy()
    t_conn[t_conn>0]=1
    bc_perm_cu = ConnectivityMatrix('permutated_btn_claw', t_conn, col_ids, row_ids)
    bk_perm_cu = bc_perm_cu.combine(ana.kc_mapping.segment_skid, 'bouton_kc_permutated_cu', axis=0)
    gk_perm_cu = bk_perm_cu.combine(ana.pn_mapping.segment_type, 'glom_kc_permutated_cu')
    return gk_perm_cu

ana = ana_all_rd
local_conn_obj = randomize_neighbours(ana, lambda x,y: pick_random_from_neighbours(x, y, n=5))
local_conn = local_conn_obj.conn['1s']

ob_ci = get_raw_inputs(np.array(local_conn))
...
fname = '210923-localrandom_vs_randombouton_recluster_k{}.png'.format(num_clusters)
...
ax1.set_aspect("equal")
fig.set_size_inches(16,12)
# fig.savefig(new_save_path + fname, bbox_inches='tight')
plt.show()



##----------------------------
# Fig5E-F observed vs. local random model
n = 5
num_exp = 10000
ana = ana_all_rd
...
for reorder_idx in [km_cluster(cm_zs.conn),reorder(ClusterOrder0707, glom_idx_ids)]:

    # Fig5E: km_cluster(cm_zs.conn) - recluster with k=4
    # Fig5F: reorder(ClusterOrder0707, glom_idx_ids) - use Fig3B order
